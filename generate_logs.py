...
# ════════════════════════════════════════════════════════════════════════════════
routing_loop = f"""
{ts(0)}  routeur   kernel: [NET] Route added: dst=10.0.0.0/24 via 192.168.10.1 dev eth0
{ts(1)}  routeur   kernel: [NET] Route added: dst=192.168.10.0/24 via 10.0.0.1 dev eth1
{ts(2)}  pc_a      kernel: [ICMP] ping 10.0.0.10 seq=1 ttl=64
{ts(3)}  routeur   kernel: [FWD] 192.168.10.10 → 10.0.0.10 via 192.168.10.1 (fwd)
{ts(4)}  routeur   kernel: [FWD] 10.0.0.10 → 192.168.10.10 via 10.0.0.1 (fwd)
{ts(5)}  routeur   kernel: [FWD] 192.168.10.10 → 10.0.0.10 via 192.168.10.1 (fwd)
{ts(6)}  routeur   kernel: [FWD] 10.0.0.10 → 192.168.10.10 via 10.0.0.1 (fwd)
{ts(7)}  routeur   kernel: [FWD] 192.168.10.10 → 10.0.0.10 via 192.168.10.1 (fwd)
{ts(8)}  routeur   kernel: [DROP] TTL exceeded in transit — src=192.168.10.10 dst=10.0.0.10 ttl=1
{ts(9)}  routeur   kernel: [ICMP] Time Exceeded sent to 192.168.10.10
{ts(10)} pc_a      kernel: [ICMP] ping 10.0.0.10 seq=1: Time to live exceeded
{ts(11)} routeur   kernel: [DROP] TTL exceeded in transit — src=192.168.10.10 dst=10.0.0.10 ttl=1
{ts(12)} routeur   kernel: [DROP] TTL exceeded in transit — src=192.168.10.10 dst=10.0.0.10 ttl=1
...
{ts(15)} pc_a      kernel: [ICMP] ping 10.0.0.10 seq=5: Destination Host Unreachable
"""
...
{ts(4)}  pc_b      kernel: [ARP] who-has 10.0.0.10? tell 192.168.10.11
{ts(6)}  pc_b      kernel: [ARP] Request timeout for 10.0.0.10 (no reply)
{ts(8)}  pc_b      kernel: [ICMP] ping 10.0.0.10 seq=1: Network unreachable
{ts(10)} pc_b      kernel: [ROUTE] no route to host 10.0.0.10 — checking local table
{ts(11)} pc_b      kernel: [ROUTE] local net: 192.168.0.0/16 — 10.0.0.10 considered LOCAL
{ts(12)} pc_b      kernel: [ARP] broadcast for 10.0.0.10 on 192.168.10.0 — no response
{ts(13)} pc_b      kernel: [ICMP] ping 10.0.0.10 seq=2: Destination Host Unreachable
{ts(14)} pc_b      kernel: [ICMP] ping 10.0.0.10 seq=3: Destination Host Unreachable
{ts(15)} pc_b      NetworkManager: WARN  gateway 192.168.10.1 unreachable (wrong subnet context)
...
# ════════════════════════════════════════════════════════════════════════════════
arp_poison = f"""
{ts(0)}  pc_a      kernel: [ARP] cache entry: 192.168.10.1 → de:ad:be:ef:00:01 (routeur)
{ts(1)}  attacker  kernel: [ARP] gratuitous ARP: 192.168.10.1 is-at ff:ff:00:00:00:01 (FAKE)
{ts(2)}  pc_a      kernel: [ARP] updated cache: 192.168.10.1 → ff:ff:00:00:00:01
{ts(3)}  pc_a      kernel: [ETH] frame sent to ff:ff:00:00:00:01 — no response
{ts(4)}  pc_a      kernel: [ICMP] ping 10.0.0.10 seq=1: Destination Host Unreachable
{ts(5)}  pc_a      kernel: [ETH] frame sent to ff:ff:00:00:00:01 — no response
{ts(6)}  pc_a      kernel: [ICMP] ping 10.0.0.10 seq=2: Destination Host Unreachable
{ts(7)}  switch_lan kernel: [SEC] duplicate MAC detected on port 3 and port 7 for IP 192.168.10.1
{ts(8)}  switch_lan kernel: [SEC] WARN possible ARP spoofing — IP 192.168.10.1 maps to 2 MACs
{ts(10)} pc_b      kernel: [ARP] cache: 192.168.10.1 → ff:ff:00:00:00:01 (STALE/POISONED)
{ts(11)} pc_b      kernel: [ICMP] ping 10.0.0.10 seq=1: Destination Host Unreachable
{ts(12)} routeur   arpwatch: WARN  flip-flop: 192.168.10.1 de:ad:be:ef:00:01 → ff:ff:00:00:00:01
...
{ts(2)}  pc_a      kernel: [ICMP] ping -s 1400 10.0.0.10 seq=1 DF=1
...
{ts(6)}  pc_a      kernel: [ICMP] ping -s 1400 10.0.0.10 seq=2 — fragmented (548+852)
...
{ts(9)}  pc_a      kernel: [TCP] connection to 10.0.0.10:80 hangs — MSS negotiated 1460 but MTU=576
...
{ts(13)} pc_a      kernel: [TCP] ERROR connection timeout to 10.0.0.10:80
{ts(14)} pc_a      kernel: [NET] WARN PMTUD blackhole detected on path to 10.0.0.10
"""
...
{ts(4)}  pc_b      kernel: [ICMP] ping 10.0.0.10 seq=1: Network unreachable
{ts(5)}  pc_b      kernel: [ROUTE] no route to host 10.0.0.10
{ts(6)}  pc_b      kernel: [ICMP] ping 10.0.0.10 seq=2: Network unreachable
{ts(7)}  pc_b      kernel: [TCP] connect to 10.0.0.10:80: Network unreachable
...
{ts(1)}  pc_a      kernel: [TCP] bulk transfer to 10.0.0.10:80 — window=65535
{ts(2)}  pc_b      kernel: [TCP] bulk transfer to 10.0.0.10:80 — window=65535
...
{ts(7)}  pc_a      kernel: [TCP] retransmit — 3 dup ACK from 10.0.0.10 (fast retransmit)
